...
public partial class MyMesh : MonoBehaviour {
    ...
    {
        Vector3[] triNormal = new Vector3[tri_amount];
        int index = 0;
        for (int i = 0; i < triNormal.Length; i++)
        {
            triNormal[i] = FaceNormal(v, t[index], t[index + 1], t[index + 2]);
            index += 3;
        }
        ...
        for(int j = 0; j < n.Length; j++)
        {
            ...
            int counter = 0;
            int triNormal_index = 0;
            while (counter < t.Length)
            {
                if (t[counter] == j || t[counter + 1] == j || t[counter + 2] == j)
                {
                    vec += triNormal[triNormal_index];
                }
                triNormal_index++;
                counter += 3;
            }
