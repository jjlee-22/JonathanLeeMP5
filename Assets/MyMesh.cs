...
public partial class MyMesh : MonoBehaviour {
    ...
    void initMesh(int N, int M)
    {
        ...
        for(int m = 0; m < M; m++)
        {
            ...
            {
                v[index] = new Vector3(-size + n1 * N_dif, 0, -size + m * M_dif);
                index++;
            }
        }
    ...
    void GetUV(Vector2[] uv)
    {
        float num = 2f / (float) (size - 1);
        for (int y = 0; y < size; ++y)
        {
            for (int x = 0; x < size; ++x)
            {
                int vertexIndex = x + y * size;
    ...
    void InitializeTextureTransform() => mUVTransform = new GameObject("Texture Transform").transform;
}
